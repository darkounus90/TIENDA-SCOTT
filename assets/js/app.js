  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const MAX_WIDTH = 800; // Un tamaño razonable para e-commerce
        const MAX_HEIGHT = 800;
        let width = img.width;
        let height = img.height;

        // Calcular nueva proporción si se excede el límite
        if (width > height) {
          if (width > MAX_WIDTH) {
        ...
        // Poner blanco como fondo (para png transparentes)
        ctx.fillStyle = '#FFFFFF';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        
        ctx.drawImage(img, 0, 0, width, height);
        
        // Comprimir a JPEG con calidad 0.7 para reducir peso dramáticamente
